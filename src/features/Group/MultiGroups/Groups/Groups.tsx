    ...
    useEffect(() => {
        if (apiData && filters) {
            if (filters === initFilters) {
                ...
                const groups = apiData.filter((el) => {
                    if (filters.style !== "") {
                        const elStyle =
                            typeof el.danceStyle === "string"
                                ? el.danceStyle
                                : el.danceStyle.name;
                        if (elStyle !== filters.style) return false;
                    }
                    if (filters.level !== "") {
                        if (el.danceLevel !== filters.level) return false;
                    }
                    if (filters.location !== "") {
                        const elLocation =
                            typeof el.location === "string"
                                ? el.location
                                : el.location.adress;
                        if (elLocation !== filters.location) return false;
                    }
                    if (filters.gender !== "") {
