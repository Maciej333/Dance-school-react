...
export default function GroupFilter(props: { setFilters: (data: Object) => void }) {
    ...
    const hangleChange = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    return (
        <form className='form' onSubmit={handleSubmit}>
            ...
            <select id="style" name="style" value={formData.style || ""} onChange={hangleChange}>
                <option value={""}>***</option>
                {
                    optionsStyle.map((el, id) => {
                        return <option key={`[option style]=${id}`} value={el.name}>{el.name}</option>
                    })
                }
            </select>

            <label htmlFor="level">Level</label>
            <select id="level" name="level" value={formData.level || ""} onChange={hangleChange}>
                <option value={""}>***</option>
                {
                    changeEnumToArray(DanceLevel)
                        .map((el, id) => {
                            return <option key={`[option level]=${id}`} value={DanceLevel[Number(el)]}>{el + ""}</option>
                        })
                }
            </select>

            <label htmlFor="location">Location</label>
            <select id="location" name="location" value={formData.location || ""} onChange={hangleChange}>
                <option value={""}>***</option>
                {
                    optionsLocation.map((el, id) => {
                        return <option key={`[option location]=${id}`} value={el.adress}>{el.adress}</option>
                    })
                }
            </select>

            <label htmlFor="gender">Gender</label>
            <select id="gender" name="gender" value={formData.gender || ""} onChange={hangleChange}>
                <option value={""}>***</option>
                {
                    changeEnumToArray(Gender)
                        .map((el, id) => {
                            return <option key={`[option gender]=${id}`} value={DanceLevel[Number(el)]}>{el + ""}</option>
                        })
                }
            </select>
